...
export class CriarCarroComponent implements OnInit {
  ...
  enviar(){
    ...
    if(valid){
      this.service.createCarro(this.carro).subscribe(carro => {
        let id: any  = carro.id
        this.criarImagem();
        this.carro.fotos = id;
        this.service.updateCarro(this.carro, id).subscribe(() => {
  ...
  criarImagem() {
    ...
    this.service.createImgCarro(this.formData).subscribe(() => {
    });
  }
