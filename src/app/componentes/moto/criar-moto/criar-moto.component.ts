...
export class CriarMotoComponent implements OnInit {
  ...
  constructor(private service: ConectService ,private serviceRequisitos: RequisitosFormularioMotoService, private sanitizer: DomSanitizer, private router: Router) { }
  ...
  criarImagem() {
    ...
    this.service.createImgMoto(this.formData).subscribe(() => {
    });
  }
  ...
  enviar(){
    ...
    if(valid){
      this.service.createMoto(this.moto).subscribe(moto => {
        let id: any = moto.id
        this.criarImagem()
        this.moto.fotos = id
        this.service.updateMoto(this.moto, id).subscribe(()=>{
