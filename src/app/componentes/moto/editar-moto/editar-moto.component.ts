...
export class EditarMotoComponent implements OnInit {
  ...
  constructor(private service: ConectService, private router: Router,private serviceRequisitos: RequisitosFormularioMotoService, private sanitizer: DomSanitizer) { }
  ngOnInit(): void {
    ...
    this.service.readMotoId(this.recuperarIdUrl()).subscribe(motos => {
      this.moto = motos
  ...
  atualizar(){
    ...
    if(valid){
      this.criarImagem()
      this.service.updateImgMoto(this.formData,this.recuperarIdUrl()).subscribe(imagem => {
        this.moto.fotos = this.recuperarIdUrl()
        this.service.updateMoto(this.moto, this.recuperarIdUrl()).subscribe(()=>{
  ...
  desativar(){
    this.service.deleteMoto(this.recuperarIdUrl()).subscribe(() => {
      this.service.deleteImgMoto(this.recuperarIdUrl()).subscribe(() => {
        this.router.navigate(['/moto'])
      })
    });
  }
